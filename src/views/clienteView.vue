<template>
  <div class="cliente">
      ...
      <v-card-title>
        <v-row class="my-4" justify-sm="start" justify="space-between">
          ...
          <v-col
            ...
            sm="12"
            cols="auto"
            class="align-self-end text-sm-start text-md-end text-lg-end"
          >
            <v-btn
              prepend-icon="mdi mdi-file-upload"
              color="yellow"
              @click="dialogArchivo = true"
              >Subir archivo</v-btn
            >
          </v-col>
          <v-col
            ...
            sm="12"
            cols="auto"
            class="align-self-end text-sm-start"
          >
            <v-btn
        ...
        <!-- eslint-disable-next-line vue/valid-v-slot -->
          <template v-slot:item.actions="{ item }">
            <v-icon
              size="small"
              class="me-2"
              @click="verClienteFunction(Object.assign({}, item))"
            >
            ...
            <v-icon
              size="small"
              class="me-2"
              @click="preEditarCliente(Object.assign({}, item))"
            >
              mdi-pencil
            </v-icon>
      ...
      <v-card>
              ...
              <v-row>
                <v-col cols="12">
                  <v-text-field
                    ...
                    min="1"
                    required
                    hint="Sin comas o puntos (, .)"
                    persistent-hint
                    variant="outlined"
                    v-model="formClienteEditar.documento"
                    :rules="numberRules"
                  ></v-text-field>
                </v-col>
                <v-col cols="12" sm="6">
                  <v-text-field
                    ...
                    required
                    variant="outlined"
                    v-model="formClienteEditar.nombres"
                    :rules="nombreRules"
                  ></v-text-field>
                </v-col>
                <v-col cols="12" sm="6">
                  <v-text-field
                    ...
                    required
                    variant="outlined"
                    v-model="formClienteEditar.apellidos"
                    :rules="nombreRules"
                  ></v-text-field>
                </v-col>
                <v-col cols="12" sm="6">
                  <v-text-field
                    ...
                    required
                    variant="outlined"
                    v-model="formClienteEditar.telefono"
                    :rules="nombreRules"
                  ></v-text-field>
                </v-col>
                <v-col cols="12" sm="6">
                  <v-text-field
                    ...
                    required
                    variant="outlined"
                    v-model="formClienteEditar.correo"
                    :rules="nombreRules"
                  ></v-text-field>
                </v-col>
                <v-col cols="12" sm="6">
                ...
                <v-col cols="12" sm="6">
                  <v-autocomplete
                    ...
                    label="Ruta"
                    required
                    v-model="formClienteEditar.direccion"
                    item-title="nombre"
                    item-value="_id"
        ...
        <v-card-actions class="justify-end">
          <v-btn
            color="red-darken-1"
            variant="tonal"
            @click="dialogClienteEditar = false"
          >
            Cerrar
          </v-btn>
          <v-btn
              ...
              <v-row>
                ...
                <v-col cols="12" sm="6">
                  <v-text-field
                    ...
                    label="Ruta"
                    required
                    v-model="verCliente.direccion.nombre"
                    disabled
                  ></v-text-field>
                </v-col>
                <v-col cols="12" sm="6">
                  <v-text-field
                    ...
                    label="Departamento"
                    required
                    v-model="verCliente.direccion.departamento"
                    disabled
                  ></v-text-field>
                </v-col>
                <v-col cols="12" sm="6">
                  <v-text-field
                    ...
                    label="Ciudad"
                    required
                    v-model="verCliente.direccion.ciudad"
                    disabled
                  ></v-text-field>
                </v-col>
                <v-col cols="12" sm="6">
                  <v-text-field
                    ...
                    required
                    v-model="verCliente.direccion.opcRuta"
                    disabled
                  ></v-text-field>
                </v-col>
                <v-col
                  cols="12"
                  sm="6"
                  v-if="verCliente.direccion.opcRuta == 'Semanal'"
                >
                  <v-text-field
                    variant="outlined"
                    label="Día"
                    required
                    disabled
                    v-model="verCliente.direccion.semanal"
                  ></v-text-field>
                </v-col>
                <v-col
                  cols="12"
                  sm="6"
                  v-if="verCliente.direccion.opcRuta == 'Quincenal'"
                >
                  <v-text-field
                    variant="outlined"
                    label="Día"
                    required
                    disabled
                    v-model="verCliente.direccion.quincenal.dia"
                  ></v-text-field>
                </v-col>
                <v-col
                  cols="12"
                ...
                >
                  <v-text-field
                    ...
                    required
                    disabled
                    v-model="verCliente.direccion.quincenal.semanas[0]"
                  ></v-text-field>
                </v-col>
                <v-col
                  cols="12"
                ...
                >
                  <v-text-field
                    ...
                    required
                    disabled
                    v-model="verCliente.direccion.quincenal.semanas[1]"
                  ></v-text-field>
                </v-col>
                <v-col
                  cols="12"
                  sm="6"
                  v-if="verCliente.direccion.opcRuta == 'Mensual'"
                >
                  <v-text-field
                    variant="outlined"
                    label="Día"
                    required
                    disabled
                    v-model="verCliente.direccion.mensual.dia"
                  ></v-text-field>
                </v-col>
                <v-col
                  cols="12"
    ...
    <v-dialog v-model="dialogArchivo" persistent width="700">
      <v-card>
        <v-card-title> Subir múltiples clientes </v-card-title>
        <v-card-text>
          <v-container>
  ...
  methods: {
    ...
    async editarCliente() {
      ...
      if (valid) {
        ...
        this.formClienteEditar.documento = parseInt(
          this.formClienteEditar.documento
        );
        await axios
          .put(`${this.api}/cliente/actualizar`, this.formClienteEditar, {
            headers: {
              Authorization: `Bearer ${this.token}`,
            },
          })
    ...
    async subirExcel() {
      ...
      if (valid) {
        ...
        this.$emit(
          "loadingSweet",
          "Estamos procesando el archivo, esto puede tardar unos minutos..."
        );
        await axios
          .post(
            `${this.api}/cliente/subir`,
            { excel: this.archivoClientes[0] },
            {
              headers: {
