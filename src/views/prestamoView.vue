<template>
  <v-card>
    ...
    <v-dialog v-model="dialogoChangeRoute" persistent width="700">
      <v-card>
        <v-card-title> Cambiar ruta </v-card-title>
        <v-card-text>
  ...
  methods: {
    ...
    dialogActualizarVenta(item) {
      ...
      this.actualizarVentaAntigua.resta =
        item.total -
        item.abono.reduce(
          (acumulador, value) => acumulador + parseInt(value.monto),
          0
        );
    ...
    async actualizarVenta() {
      ...
      if (valid) {
        ...
        if (
          !this.actualizarVentaAntigua.ruta ||
          !this.actualizarVentaAntigua.producto
        ) {
          return Swal.fire({
            icon: "warning",
            text: "Debes seleccionar un barrio y un producto",
            showConfirmButton: false,
            timer: 1690,
          });
        } else if (
        ...
        await axios
          .put(`${this.api}/prestamo/actualizarVenta`, paquete, {
            headers: {
              Authorization: `Bearer ${this.token}`,
            },
          })
      ...
      if (this.$refs.formAbono.validate()) {
        if (this.abonoAdd > 0 && this.fechaAdd) {
          ...
          this.abonosTabla.push({
            fecha: nFecha.toISOString().slice(0, 10),
            monto: this.abonoAdd,
          });
          this.abonar.abonos.push({
            fecha: nFecha.toISOString().toString(),
            monto: parseFloat(this.abonoAdd),
          });
          this.fechaAdd = null;
          this.abonoAdd = 0;
        }
    ...
    async guardarAbonos() {
      if (this.$refs.formAbono.validate()) {
        const c = parseInt(
          this.abonar.abonos.length + parseInt(this.abonar.cuotas)
        );
        ...
        if (this.formaPago == 2) {
          if (this.abonar.pago_fechas.length == this.abonar.cuotas) {
            if (this.totalAbonos <= this.abonar.total) {
              ...
              this.abonar.fechaVenta = new Date(
                `${fV.getFullYear()}-${fV.getMonth() + 1}-${fV.getDate()}`
              )
                .toISOString()
                .toString();

              await axios
                .post(`${this.api}/prestamo/abonar`, this.abonar, {
                  headers: {
                    Authorization: `Bearer ${this.token}`,
                  },
                })
                .then((response) => {
                  if (response.data) {
                    ...
                    Swal.fire({
                      icon: "success",
                      text: "Abonos agregados correctamente",
                      showConfirmButton: false,
                      timer: 1600,
                    });
                  } else {
                    Swal.fire({
                      icon: "error",
                      text: "No se pudieron agregar los abonos",
                      showConfirmButton: false,
                      timer: 1650,
                    });
                  }
              ...
              Swal.fire({
                icon: "info",
                text: "El total debe ser menor o igual al total de la venta",
                showConfirmButton: false,
                timer: 1600,
              });
            }
            ...
            Swal.fire({
              icon: "info",
              text: "La cantidad de fechas de pago debe ser igual a la cantidad de cuotas",
              showConfirmButton: false,
              timer: 1600,
            });
          }
          ...
          const i = this.rutas.findIndex(
            (ruta) => ruta._id == this.abonar.ruta
          );
          this.abonar.rutaPlana = this.rutas[i].nombre;
          this.abonar.total = parseInt(this.abonar.total);
          this.abonar.documento = parseInt(this.abonar.documento);
    ...
    async llenarMes(anio, mes, dia) {
      ...
      for (let i = fechaActual.getDate(); i <= ultimoDiaMes; i++) {
        ...
        infoMes.push({
          semana: numSemana,
          dia: i,
          diaTexto: this.diasSemana[fechaAux.getDay()],
        });
      }
      return infoMes;
    },
      ...
      switch (frecuenciaCobro.toLowerCase()) {
        ...
        case "semanal":
          ...
          diaFrecuencia = this.diasSemana.findIndex(
            (dia) => dia.toLowerCase() == semana.toLowerCase()
          );
        ...
        case "quincenal":
          ...
          while (fechaPagos.length < cuotas) {
            ...
            mesV.filter((mes) => {
              if (
                quincena.semanas.includes(mes.semana) &&
                mes.diaTexto.toLowerCase() == quincena.dia.toLowerCase() &&
        ...
        case "mensual":
          ...
          while (fechaPagos.length < cuotas) {
            ...
            mesV.filter((mes) => {
              if (
                mensual.semanas == mes.semana &&
                mes.diaTexto.toLowerCase() == mensual.dia.toLowerCase() &&
                fechaPagos.length < cuotas
              ) {
                fechaPagos.push({
                  fecha: `${fechaActual.getFullYear()}-${
